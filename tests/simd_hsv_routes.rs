//! SIMD HSV batch-route correctness tests (issue #71).
//!
//! Each test generates a deterministic pixel batch, computes the scalar
//! f64 reference result per pixel via `rgb::hsv`, then calls the SIMD
//! batch function `simd_hsv::rgb_to_hsv_batch` (wide::f32x8, producing
...
//! by 360, the worst-case f32→f64 gap in hue is ~3e-4. Sat and value
...
/// Behavior 1: `rgb_to_hsv_batch` (f32x8 SIMD) must match the scalar
/// `rgb::hsv` (f64) within HSV_TOLERANCE (1e-3) for batches including
/// non-multiples of the SIMD lane width (8).
#[test]
fn rgb_to_hsv_batch_matches_scalar() {
    for n in [1, 7, 8, 15, 16, 100, 257] {
        let pixels = generate_rgb_pixels(n);
        let scalar: Vec<[f64; 3]> = pixels.iter().map(|&p| rgb::hsv(p)).collect();
        let simd_result = simd_hsv::rgb_to_hsv_batch(&pixels);
...
/// Also test primary-color edge cases and black (0,0,0).
#[test]
fn rgb_to_hsv_batch_achromatic_and_edge_cases() {
    ...
    let test_pixels: Vec<[u8; 3]> = vec![
        ...
        [128, 128, 128], // grey   → h=0, s=0, v≈50
        [255, 255, 255], // white  → h=0, s=0, v=100
        // Primary colours
        [255, 0, 0], // red    → h=0,   s=100, v=100
        [0, 255, 0], // green  → h=120, s=100, v=100
        [0, 0, 255], // blue   → h=240, s=100, v=100
        // Secondary colours
        [255, 255, 0], // yellow → h=60,  s=100, v=100
        [0, 255, 255], // cyan   → h=180, s=100, v=100
        [255, 0, 255], // magenta→ h=300, s=100, v=100
        // Edge cases
        [1, 0, 0],       // near-black red
        [254, 255, 255], // near-white with hint
    ];

    let scalar: Vec<[f64; 3]> = test_pixels.iter().map(|&p| rgb::hsv(p)).collect();
    let simd_result = simd_hsv::rgb_to_hsv_batch(&test_pixels);
